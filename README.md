...
For containerized development, copy `.env.compose.example` to `.env.compose`, populate secrets, and run `docker compose up --build`. For Kubernetes/k3s deployments, use the Helm chart under `chart/`, the raw manifests in `k8s/`, or the on-prem helper script documented in `docs/containers.md`.
